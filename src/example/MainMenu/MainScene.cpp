...
bool MainScene::HandleEvents(float fElapsedTime) {
    if (game->GetKey(olc::F).bPressed) {
        std::cout << "F key pressed"
                  << "\n";
        game->PlaySound("assets/sounds/bottle-smash-101001.mp3",
                        &game->audio_sfx);
    }
    ...
    if (game->GetKey(olc::ENTER).bPressed ||
        game->GetKey(olc::RIGHT).bPressed) {
...
bool MainScene::Update(float fElapsedTime) { return true; }
...
bool MainScene::Draw(float fElapsedTime) {
    ...
    if (state->TopScene() == this) {
        game->DrawRect(vOptionsOffset.x - 10,
                       vOptionsOffset.y - nOptionSpacing / 2, 100,
