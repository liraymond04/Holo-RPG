...
bool PlayScene::HandleEvents(float fElapsedTime) {
    vec3d vUp = { 0, 1, 0 };
    vec3d vForward =
        e3d.camera->vLookDir * (e3d.camera->fMoveSpeed * fElapsedTime);
    ...
    e3d.camera->fPitch =
        pitch_clamp(e3d.camera->fPitch, -PI_BY_2, PI_BY_2, 0.001f);
...
bool PlayScene::Draw(float fElapsedTime) { return true; }