...
bool Engine3D::Update(float fElapsedTime)
{
    ...
    {
        gameObject->scale.x += camera->fTurnSpeed * fElapsedTime * pow(-1, game->GetKey(olc::CTRL).bHeld);
    }
    if (game->GetKey(olc::Y).bHeld)
    {
        gameObject->scale.y += camera->fTurnSpeed * fElapsedTime * pow(-1, game->GetKey(olc::CTRL).bHeld);
    }
    if (game->GetKey(olc::Z).bHeld)
    {
        gameObject->scale.z += camera->fTurnSpeed * fElapsedTime * pow(-1, game->GetKey(olc::CTRL).bHeld);
    }
